...
Most scripts using input data that are publicly available include code to download those data dynamically. Some external data that are publicly-available are included in the repository's data directory `./data/`. External source files that are restricted access need to be manually added to `./wd/in/` locally. 

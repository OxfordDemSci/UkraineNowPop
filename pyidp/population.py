...
def agesex_cols(gender, age_min, age_max):
    ...
    result = []
    for g in gend:
        result += [g + '_' + '_'.join([str(x).zfill(2), str(x + 4).zfill(2)]) for x in list(range(age_min, age_max, 5))]
...
def baseline_audience(base_date, country, geo_level, geo_keys, platform='facebook',
                      agesex=agesex_cols(gender=[2, 1], age_min=15, age_max=65),
                      language_name='all', location_types='["recent"]'):
    """
    ...
    for idx, row in result.iterrows():
        ...
        while len(api_response) == 0 & delta_date < 10:
            ...
            args.update({'date_start': query_date.strftime('%Y-%m-%d'),
                         'date_end': query_date.strftime('%Y-%m-%d')})
            api_response = pyidp.query_api(endpoint='query_clean', args=args)
...
def daily_audience(date_start, date_end, country, geo_level, geo_keys, platform='facebook',
                      agesex=agesex_cols(gender=[2, 1], age_min=15, age_max=65),
    ...
    for geo_key in geo_keys:
        ...
        for demgroup in agesex:
            ...
            response = pyidp.query_api(endpoint='query_clean', args=args)

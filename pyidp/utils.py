...
def db_engine(host=os.environ.get('smaDB_host'),
    ...
    engine_string = 'postgresql+psycopg2://' + user + ':' + pw + '@' + host + ':' + port + '/' + db
...
def query_sql(con, sql, geom_col=None):
    try:
        if geom_col is None:
            ...
            result = gpd.read_postgis(sql=sql, con=con, geom_col=geom_col, crs='EPSG:4326')
...
def query_api(endpoint, args, max_attempts=3, url='http://18.135.72.18/api/v1/', token=os.getenv('sma_API_token')):
    ...
    while response.status_code == 429 and attempts <= max_attempts:
        logger.warning(f'Too many API calls (attempt: {attempts}). Sleeping for 60 seconds before trying again...')
        time.sleep(60)
        response = requests.get(url=url + endpoint, params=args)
        attempts += 1
...
def config_to_api_args(config, date_start, date_end):
    ...
    for idx, row in config.iterrows():
        args_item = dict(row[['collection_name', 'country', 'geo_level', 'gender', 'age_min', 'age_max',
                              'location_types', 'language_name']])

        args_item.update({'date_start': date_start,
                          'date_end': date_end,
                          'add_geometry': False,

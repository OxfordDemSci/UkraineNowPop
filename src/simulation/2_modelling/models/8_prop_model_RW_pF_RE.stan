...
parameters {
  ...
  // Facebook
  vector<lower=0>[T * I] log_p_F; // Facebook user ratios
  real mu0_p_F;
  // vector[T-1] mu_p_F_t; // time effect
  vector[T] mu_p_F_t; // time effect
  vector[I] mu_p_F_i; // location effect
  real log_sigma_p_F_t;
  real log_sigma_p_F_i;
  real log_sigma_p_F;
  // real log_nu; // student t DoF
  
  // Instagram
  // real log_p_GF;
  vector<lower=0>[T * I] log_p_GF;
  real mu_p_GF;
  real log_sigma_p_GF;
...
transformed parameters {
  ...
  // vector[T*I] p_F = exp(mu0_p_F + to_vector(mu_p_F_t * mu_p_F_i'));   
  vector<lower=0>[T * I] p_GF = exp(log_p_GF);
  // real<lower=0> p_GF = exp(log_p_GF); 
  vector<lower=0>[T * I] p_G = p_F .* p_GF;  
  // vector[T*I] mu_p_F = mu0_p_F + to_vector(append_row(0,mu_p_F_t) * mu_p_F_i');
  vector[T*I] mu_p_F = mu0_p_F + to_vector(mu_p_F_t * rep_vector(1,I)' + rep_vector(1,T) * mu_p_F_i');
  // vector[T*I] mu_p_F = mu0_p_F + to_vector(rep_vector(1,T) * mu_p_F_i');    
  ...
  for (t in 2 : T) {
    ...
    // ar(1) model on total pop
    // log_lik[t-1] = lognormal_lpdf(y_N_tot[t] | mu_tot + ar_tot*(log(y_N_tot[t-1]) - mu_tot),exp(log_sigma_tot)); 
    
    // RW model on total pop
    log_lik[t-1] = lognormal_lpdf(y_N_tot[t] | log(y_N_tot[t-1]) - mu_tot,exp(log_sigma_tot)); 
  }
...
model {
  ...
  // log_p_F ~ student_t(exp(log_nu),mu_p_F,exp(log_sigma_p_F));
  // mu0_p_F ~ normal(log(0.2),0.5);
  log_p_GF ~ normal(mu_p_GF,exp(log_sigma_p_GF));
  mu_p_F_t ~ normal(0, exp(log_sigma_p_F_t));
  // mu_p_F_t ~ student_t(exp(log_nu),0, exp(log_sigma_p_F_t));
...
generated quantities {
  ...
  y_N_tot_hat[1] = lognormal_rng(log(y_N_tot[2]) + mu_tot,exp(log_sigma_tot));
  y_N_tot_hat[2:T] = lognormal_rng(log(y_N_tot[1:(T-1)]) -  mu_tot,exp(log_sigma_tot));

...
model_data <- function(
    ...
    codps_N2, confidence_N2, date_N2,
    last_date,
  ...
  time_index_expanded <- tibble(
    collection_date = seq(as.Date(date_start), as.Date(last_date), by = 1)
  ) |>
    mutate(
  ...
  pop1_week <- floor_date(as.Date(date_N1), "week", week_start = 1)
  t_pop1 <- time_index %>%
  ...
  pop2_week <- floor_date(as.Date(date_N2), "week", week_start = 1)
  t_pop2 <- time_index %>%

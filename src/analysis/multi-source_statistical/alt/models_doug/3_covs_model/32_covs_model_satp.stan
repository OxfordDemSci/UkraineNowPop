...
parameters {
  ...
  vector[T] delta_p_F;
  real<lower=0> sigma_delta_p_F;
  
  vector[I] gamma_p_F;
  real<lower=0> sigma_gamma_p_F;
  ...
  vector[T] delta_p_G;
  real<lower=0> sigma_delta_p_G;
  
  vector[I] gamma_p_G;
  real<lower=0> sigma_gamma_p_G;
}
transformed parameters {
  ...
  mu_p_F = alpha_p_F + delta_p_F[tt] + gamma_p_F[ii] + X_p_F * beta_p_F;
  
  // regression on Facebook detection rates
  mu_p_G = alpha_p_G + delta_p_G[tt] + gamma_p_G[ii] + X_p_G * beta_p_G;
...
model {
  ...
  delta_p_F ~ normal(0, sigma_delta_p_F);
  sigma_delta_p_F ~ normal(0, 1);
  
  gamma_p_F ~ normal(0, sigma_gamma_p_F);
  sigma_gamma_p_F ~ normal(0, 1);
  ...
  delta_p_G ~ normal(0, sigma_delta_p_G);
  sigma_delta_p_G ~ normal(0, 1);
  
  gamma_p_G ~ normal(0, sigma_gamma_p_G);
  sigma_gamma_p_G ~ normal(0, 1);
}

functions {
  // Slice function for specific year t
  ...
  // Slice function for previous year t-1
...
data {
  ...
  array[T * I * G * S] int<lower=0> tt; 
  array[T * I * G * S] int<lower=0> ii; 
  array[T * I * G * S] int<lower=0> gg; 
  array[T * I * G * S] int<lower=0> ss; 
...
parameters {
  ...
  real alpha_r;              // overall intercept for population growth rates
  real<lower=0> sigma_r;     // variation in growth rates
  
  // Random Walk Component for mu_r
  vector[T] rw_r;            // random walk increments
  real mu_initial_rw_r;      // initial mean for rw_r
  real<lower=0> sigma_rw_r;  // standard deviation for random walk
  
  // Regression Coefficients for mu_r
  vector[K_r] beta_r;        // regression coefficients for covariates
  
  // Social Media Detection Ratios
  ...
  real<lower=0> sigma_p_F; // residual variation for p_F
  real<lower=0> sigma_p_G; // residual variation for p_G
...
transformed parameters {
  ...
  // Define mu_r with regression and random walk
  for (t in 1:T) {
    for (c in 1:C) {
      int idx = (t - 1) * C + c;
      mu_r[idx] = alpha_r + rw_r[t];
    }
  }
...
model {
  ...
  // Priors for Random Walk
  rw_r[1] ~ normal(mu_initial_rw_r, sigma_rw_r);
  for (t in 2:T) {
    rw_r[t] ~ normal(rw_r[t - 1], sigma_rw_r);
  }
  
  // Priors for Population Parameters
  alpha_r ~ normal(0, 5);
  sigma_rw_r ~ normal(0, 1);
  mu_initial_rw_r ~ normal(0, 1);
  sigma_r ~ normal(0, 1);
  
  // Priors for Social Media Detection Ratios
  alpha_p ~ normal(0, 5);
  phi_p ~ normal(0, 1);
  ...
  nu_r ~ normal(0, sigma_nu_r);
  sigma_nu_r ~ normal(0, 1);

  epsilon_r ~ normal(0, sigma_epsilon_r);
  sigma_epsilon_r ~ normal(0, 1);

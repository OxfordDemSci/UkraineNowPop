...
md$idx <- idx %>%
  ...
  arrange(t, i, a, s) %>%
  mutate(tias = row_number()) %>%                # full (t,i,a,s) id (kept)
  group_by(a, s) %>%
...
adult_map <- md$idx %>%
  filter(a != 1) %>%                       # drop young
  arrange(t, i, a, s) %>%
  mutate(tias_adult = row_number()) %>%    # 1 .. T * I * (A-1) * S
...
md$M <- matrix(0L, nrow=md$C_adult, ncol=I*S)
for(k in 1:md$C_adult) {
  slice_pos <- ((k-1) %% (I*S)) + 1
  md$M[k, slice_pos] <- 1L
}

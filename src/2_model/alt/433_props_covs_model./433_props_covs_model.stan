...
transformed parameters {
  ...
  for (c in 1:C) {
    for (i in 1:(T * I)) {
      ...
      p_G[c][i] = exp(log(p_F[c][i]) + phi_p);
    }
  }

  for (i in 1:n_F) {
    int c = comb_F[i];
    log_lik[i] = lognormal_lpdf(y_F[i] | log(N[c][ti_F[i]] * p_F[c][ti_F[i]]), exp(log_sigma_F));
  }

  for (i in 1:n_G) {
    int c = comb_G[i];
    log_lik[i + n_F] = lognormal_lpdf(y_G[i] | log(N[c][ti_G[i]] * p_G[c][ti_G[i]]), exp(log_sigma_G));
  }
}

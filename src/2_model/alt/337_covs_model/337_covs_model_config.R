...
selected_locations <- seq(1, 27, by = 1#c(#1, #2
                        #4, #6
                        #7, #6
                        #16, #4
                        #20, #5
...
selected_locations_key <- c(3788, 3800, 3801, 3781, 3782, 3804, 3802, 3803, 3783, 3790, 3787, 3791, 3792, 3793, 3794, 3795, 3796, 3798,
...
#i_idx <- idx %>% 
  ...
  select(ti, as, tias, t, i, a, s, t_key, t_name, i_key, i_name, a_key, a_name, s_key, s_name, e, e_name)
...
md$idx_F <- idx_F |>
    ...
    dplyr::select(ti, as, tias, t, i, a, s, m, value) %>%
    arrange(t, i, a, s)
...
cols_to_pivot <- c("F_20_24","F_25_29","F_30_34","F_35_39","F_40_44","F_45_49","F_50_54","F_55_59","F_60_64","F_65_69","F_70_74","F_75_79","F_80Plus",
                   "M_20_24","M_25_29","M_30_34","M_35_39","M_40_44","M_45_49","M_50_54","M_55_59","M_60_64","M_65_69","M_70_74","M_75_79","M_80Plus")
...
full_pop <- codps %>%
  ...
  pivot_longer(all_of(cols_to_pivot),
               ...
               values_to = "pop0") %>%
  filter(sex != "T", age_min != "TL")
total_pop_all <- sum(full_pop$pop0, na.rm=TRUE)
total_pop_sel <- full_pop %>%
  filter(fb_key %in% selected_locations_key) %>%
  summarize(sum_sel = sum(pop0, na.rm=TRUE)) %>%
  pull(sum_sel)
prop_sel <- total_pop_sel / total_pop_all

md$y_N_tot <- as.integer( (sum(md$N01) - weekly_avg$avg_value) * prop_sel )
md$y_N_tot[1] <- as.integer( sum(md$N01) * prop_sel )
...
init_generator <- function(md, chain_id = 1) {
  TIAS <- md$T * md$I * md$A * md$S         
  RS   <- md$A * md$S                       

  set.seed(1000 + chain_id)                 
  p0 <- if (length(md$y_F) && length(md$N0)) {
          mean(md$y_F) / mean(md$N0)
        } else 0.05
  p0 <- min(max(p0, 1e-3), 0.99)             

  list(
    r        = rlnorm(TIAS, 0, 0.05),        
    alpha_r  = rnorm(1, 0, 0.30),
    beta_r   = if (md$K_r > 0)
                 matrix(rnorm(RS * md$K_r, 0, 0.10),
                        nrow = RS, ncol = md$K_r)
               else
                 matrix(0, nrow = RS, ncol = 0),
    sigma_r  = runif(1, 0.10, 0.40),       
    alpha_p  = qlogis(p0),                 
    phi_p    = rnorm(md$A, 0, 0.20),
    beta_p   = if (md$K_p > 0) rnorm(md$K_p, 0, 0.20) else numeric(0),

    sigma_F  = runif(1, 0.10, 0.40),       
    sigma_G  = runif(1, 0.10, 0.40),

    log_sigma_delta_p = log(runif(1, 0.05, 0.30)),
    log_sigma_gamma_i = log(runif(1, 0.05, 0.30)),
    log_sigma_gamma_a = log(runif(1, 0.05, 0.30)),
    log_sigma_gamma_s = log(runif(1, 0.05, 0.30)),

    delta_p_raw   = rnorm(md$T, 0, 1),
    gamma_p_i_raw = rnorm(md$I, 0, 1),
    gamma_p_a_raw = rnorm(md$A, 0, 1),
    gamma_p_s_raw = rnorm(md$S, 0, 1)
  )
}

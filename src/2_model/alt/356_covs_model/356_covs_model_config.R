...
md$idx_F <- idx_F %>%
  ...
  summarise(value = mean(value, na.rm = TRUE), .groups = "drop") %>%  ungroup() %>%
...
mutate(a = case_when(age_group10y=="00-19"~6, age_group10y=="20-29"~1, age_group10y=="30-39"~2, age_group10y=="40-49"~3, age_group10y=="50-59"~4, age_group10y=="60-999"~5) %>% as.integer(),
...
md$N210 <- codps24 %>%
  ...
  mutate(a = case_when(age_group10y=="00-19"~6, age_group10y=="20-29"~1, age_group10y=="30-39"~2, 
  age_group10y=="40-49"~3, age_group10y=="50-59"~4, age_group10y=="60-999"~5) %>% as.integer(),
    s = ifelse(sex == "F", 2, 1) %>% as.integer()) %>%
  left_join(i_idx) %>%
  arrange(i, a, s) %>%
  mutate(ias = row_number()) %>%
...
children <- md$N010 %>% filter(a==6) %>% mutate(t=1) %>% rename(children=value) %>%
  rbind(md$N110 %>% filter(a==6) %>% mutate(t=t_pop1) %>% rename(children=value)) %>%
  rbind(md$N210 %>% filter(a==6) %>% mutate(t=t_pop2) %>% rename(children=value))

female_repo <- md$N010 %>% filter((a==1|a==2|a==3), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=1) %>%
  rbind(md$N110 %>% filter((a==1|a==2|a==3), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=t_pop1)) %>%
  rbind(md$N210 %>% filter((a==1|a==2|a==3), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=t_pop2))

#young <- md$N010 %>% filter(a==2) %>% mutate(t=1) %>% rename(young=value) %>%
#  rbind(md$N110 %>% filter(a==2) %>% mutate(t=t_pop1) %>% rename(young=value)) %>%
#  rbind(md$N210 %>% filter(a==2) %>% mutate(t=t_pop2) %>% rename(young=value))

#female_repo <- md$N010 %>% filter((a==3|a==4|a==5|a==6), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=1) %>%
#  rbind(md$N110 %>% filter((a==3|a==4|a==5|a==6), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=t_pop1)) %>%
#  rbind(md$N210 %>% filter((a==3|a==4|a==5|a==6), s==2) %>% group_by(i) %>% summarise(female_repo = sum(value, na.rm=T)) %>% ungroup() %>% mutate(t=t_pop2))
...
mutate(ratio_children_femala_im = zoo::na.approx(ratio_children_female, x = t, na.rm = FALSE)) %>%
ungroup() %>%
arrange(t, i, s) %>%
select(ratio_children_femala_im) |>
pull()
...
md$slice_young <- matrix(rep(md$young_pos_full, times = T),
                         nrow = T, byrow = TRUE)
                         md$slice_young_lag  <- rbind(rep(1L, I * S), md$slice_young[-T, ])



f_core <- (md$ss == 2L) & (md$aa %in% 1:3)     
md$idx_female_a234 <- which(f_core)            
md$idx_female_triple <- matrix(md$idx_female_a234, ncol = 3, byrow = TRUE) # dims (T*I) × 3
...
standardise_cols <- function(df, vars) {
  ...
  list(df = df, stats = setNames(stats, vars))
}
...
Xr_df <- std_r$df     # scaled data-frame
md$X_r_means  <- sapply(std_r$stats, `[[`, "mean")
md$X_r_sds    <- sapply(std_r$stats, `[[`, "sd")

md$X_r <- as.matrix(Xr_df["pwtt_sum_24week_none_none"])
md$K_r <- ncol(md$X_r)
...
# md$X_r <- as.matrix(select(Xr_df, all_of(cov_names)))
# md$K_r <- ncol(md$X_r)

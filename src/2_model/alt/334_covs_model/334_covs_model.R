...
i_idx <- idx %>% 
  ...
  select(ti, as, tias, t, i, a, s, t_key, t_name, i_key, i_name, a_key, a_name, s_key, s_name)
...
X_r <- md$idx %>% select(t, i)

for (k in seq_len(nrow(rcov_select))) {
  cov_k    <- rcov_select[k, ]
  col_name <- paste0("x", k)
  vals <- covs %>%
    filter(
      covariate == cov_k$covariate,
      sum_stat  == cov_k$sum_stat,
      time_std  == cov_k$time_std,
      space_std == cov_k$space_std
    ) %>%
    select(t, i, value_std) %>%
    rename(!!col_name := value_std)

  X_r <- X_r %>% left_join(vals, by = c("t", "i"))
}

md$X_r <- as.matrix(X_r %>% select(starts_with("x")))
md$K_r <- ncol(md$X_r)
...
X_p <- md$idx %>% select(t, i)

for (k in seq_len(nrow(pcov_select))) {
  cov_k    <- pcov_select[k, ]
  col_name <- paste0("x", k)
  vals <- covs %>%
    filter(
  ...
  X_p <- X_p %>% left_join(vals, by = c("t", "i"))
}

md$X_p <- as.matrix(X_p %>% select(starts_with("x")))
md$K_p <- ncol(md$X_p)

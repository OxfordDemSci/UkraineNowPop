...
data {
  ...
  array[T * I - I] int<lower=0> tias_N; // ti (year, location) index for N[t]       //????dimensions
  array[T * I - I] int<lower=0> tias_N_lag; // ti (year, location) index for N[t-1]
...
parameters {
  ...
  vector[T] mu_p_F; // expected value                             //????????????why is this different from dimensions of mu_p_G?
  real mu_mu_p_F;
...
model {
  ...
  // observation models
  p_F ~ lognormal(mu_p_F[tt], sigma_p_F);   //?????????hy is tt for F and ii for G

...
plot_df <- data.frame(
  ...
  separate(indices, into = c("s", "ti"), sep = ",", convert = TRUE) %>%
  left_join(md$idx, by = c("s", "ti"))
...
for (name in unique(plot_df$i_name)) {
  ...
  for (param in params) {
    ...
    p <- ggplot(df_param, aes(x = t_name, y = predicted, group = s_name)) +
      geom_ribbon(aes(ymin = lower, ymax = upper, fill = s_name), alpha = 0.2, colour = NA) +
      geom_line(aes(colour = s_name)) +
...
for (b in seq_len(nrow(grid_info_all))) {
  ...
  pars_batch <- unlist(lapply(pars_model, function(p) {
    sapply(1:md$S, function(s) paste0(p, "[", s, ",", b, "]"))
  }))

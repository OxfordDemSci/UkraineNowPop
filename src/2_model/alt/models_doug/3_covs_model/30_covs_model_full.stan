...
parameters {
  ...
  vector[I] alpha_r; // random intercept for population growth rates
  real mu_alpha_r;
  ...
  vector[T] alpha_p_F; // random intercept on Facebook detection rates
  real mu_alpha_p_F;
  real<lower=0> sigma_alpha_p_F;
  ...
  vector[I] alpha_p_G; // random intercept on Instagram detection rates
  real mu_alpha_p_G;
  real<lower=0> sigma_alpha_p_G;
}
transformed parameters {
  ...
  mu_r = alpha_r[ii] + X_r * beta_r;
  
  // regression on Facebook detection rates
  mu_p_F = alpha_p_F[tt] + X_p_F * beta_p_F;
  
  // regression on Facebook detection rates
  mu_p_G = alpha_p_G[ii] + X_p_G * beta_p_G;
...
model {
  ...
  // priors:  population
  alpha_r ~ normal(mu_alpha_r, sigma_alpha_r);
  mu_alpha_r ~ normal(0, 1);
  sigma_alpha_r ~ normal(0, 1);

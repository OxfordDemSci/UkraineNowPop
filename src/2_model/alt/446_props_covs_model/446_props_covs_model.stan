...
model {
  // log‐likelihood
  for (n in 1:n_F) {
    target += lognormal_lpdf(y_F[n] | log( N[idx_F[n]] * p_F[idx_F[n]] ), exp(log_sigma_F));
  }
  
  for (m in 1:n_G) {
    target += lognormal_lpdf(y_G[m] | log( N[idx_G[m]] * p_G[idx_G[m]] ), exp(log_sigma_G));
  }


  //logit_pi_free[1] ~ normal(rep_vector(0, C-1), 5);
  //for (t in 2:(T-1))
  //  logit_pi_free[t] ~ normal(logit_pi_free[t-1], exp(log_sigma_pi));  //check if RE help

  //log_sigma_pi ~ normal(0, 1);
  ...
  r ~ normal( alpha_r
              ...
              //+ gamma_r_a[aa]
              //+ gamma_r_s[ss]
              + X_r * beta_r, exp(log_sigma_r) );


  delta_p ~ normal(0, exp(log_sigma_delta_p));
  gamma_p_i ~ normal(0, exp(log_sigma_gamma_p_i));
  gamma_p_a ~ normal(0, exp(log_sigma_gamma_p_a));
  gamma_p_s ~ normal(0, exp(log_sigma_gamma_p_s));

  delta_r ~ normal(0, exp(log_sigma_delta_r));
  gamma_r_i ~ normal(0, exp(log_sigma_gamma_r_i));
  gamma_r_a ~ normal(0, exp(log_sigma_gamma_r_a));
  gamma_r_s ~ normal(0, exp(log_sigma_gamma_r_s));
}
...
generated quantities {
  ...
  for (n in 1:n_F)
    F_hat[n] = lognormal_rng(log(N[idx_F[n]] * p_F[idx_F[n]]), exp(log_sigma_F));
  
  for (m in 1:n_G)
    G_hat[m] = lognormal_rng(log(N[idx_G[m]] * p_G[idx_G[m]]), exp(log_sigma_G));
}

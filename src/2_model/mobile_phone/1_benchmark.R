...
plot_nal <- comparison23 %>%
  ...
  ungroup() %>%
  mutate(
    pop_estimated_nal = ifelse(s_name == "M", pop_estimated_nal * (-1), pop_estimated_nal),
...
plot_obl <- comparison23 %>%
  ...
  ungroup() %>%
  mutate(
    pop_estimated_obl = ifelse(s_name == "M", -pop_estimated_obl, pop_estimated_obl),
...
ggplot(plot_obl, aes(x = a_name)) +
  ...
  scale_fill_manual(
    values = c(F = "#8ECAE6", M = "#219EBC"),
    name = NULL,
    labels = c(F = "Female (Vodafone est.)", M = "Male (Vodafone est.)")
  ) +
  scale_colour_manual(
    values = c(F = "#005F73", M = "#D00000"),
    name = NULL,
    labels = c(F = "Female (COD-PS 2023)", M = "Male (COD-PS 2023)")
  ) +
  labs(
    title = "Vodafone estimates vs. COD‑PS 2023 population pyramid (1 July 2023)", x = NULL,
...
ggplot(plot_nal, aes(x = a_name)) +
  ...
  scale_y_continuous(labels = function(x) formattable::comma(abs(x))) +
  labs(
    title = "Vodafone estimates vs. COD‑PS 2024 population pyramid (1 September 2024)",
    x = NULL, y = "Population"
  ) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "bottom")
...
ggplot(plot_obl, aes(x = a_name)) +
  ...
  labs(
    title = "Vodafone estimates vs. COD‑PS 2024 population pyramid (1 September 2024)", x = NULL,
    y = "Population (in thousands)"
  ) +
  theme_minimal(base_size = 15) +
  theme(legend.position = "bottom")
...
movers <- flows_hromada_agesex |>
  filter(origin_raion != destination_raion) |>
  group_by(t, destination_raion, destination_oblast) |>
  summarise(movers = sum(monthlyFlow_hat_calibrated))
...
movers <- movers |>
  ...
  left_join(
    idp |>
      mutate(
        t = as.Date(paste0("01", "-", month(as.Date(reportingDate)), "-", year(as.Date(reportingDate))), format = "%d-%m-%Y")
      ) |>
...
ggplot(movers |> filter(!is.na(idp) & t > as.Date("2022-02-01")), aes(x = t, y = diff_perc, col = destination_raion)) +
  geom_line() +
  facet_wrap(. ~ destination_oblast, scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Difference between IDP and movers: from the start of the war", y = "(Movers-IDP)/IDP (%)")

ggplot(movers |> filter(!is.na(idp) & t > as.Date("2022-03-01")), aes(x = t, y = diff_perc, col = destination_raion)) +
  geom_line() +
  facet_wrap(. ~ destination_oblast, scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Difference between IDP and movers: after the start of the war", y = "(Movers-IDP)/IDP (%)")

ggplot(movers |> filter(!is.na(idp)), aes(x = t, y = idp, col = destination_raion)) +
  geom_line() +
  facet_wrap(. ~ destination_oblast, scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "registered IDP", y = "count")

ggplot(movers |> filter(!is.na(idp)), aes(x = t, y = movers, col = destination_raion)) +
  geom_line() +
  facet_wrap(. ~ destination_oblast, scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "none") +

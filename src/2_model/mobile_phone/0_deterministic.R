...
borderCrossing_out <- read.csv(file.path(out_dir, "population_proxy", "crossing_borders", "refugees_out_daily.csv"),
  stringsAsFactors = F,
  skip = 1,
  skipNul = T,
  sep = ";"
...
national_pop_minusOutflows <- bind_rows(
  ...
  bind_rows(
    lapply(seq(as.Date("2021-11-01"), min(borderCrossing_out_monthly_agesex$t) - months(1), by = "month"), function(t) {
      agesex_national_d0 |>
        mutate(t = t)
    })
  )
...
for (idx in 1:n_distinct(monthlyFlows$t)) {
  ...
  monthlyFlows_totals_hat[[idx]] <- monthlyFlows_totals[[idx]] |>
    ...
    mutate(
      p_ = case_when(
        ...
        origin_hromada == "abroad" & is.na(origin_p) ~ mean(origin_p, na.rm = T),
        T ~ origin_p
      )
    ) |>
  ...
  monthlyFlows_agesex_hat[[idx]] <- monthlyFlows_agesex[[idx]] |>
    ...
    mutate(
      p = case_when(
        ...
        origin_macroregion == "Abroad" & is.na(origin_p) ~ mean(origin_p, na.rm = T),
        T ~ origin_p
      )
    ) |>
    ungroup() |>
  ...
  if (monthlyFlows_totals[[idx]]$t[1] >= as.Date("2022-02-01")&monthlyFlows_totals[[idx]]$t[1]< max(monthlyFlows$t)) {
...
dir.create(file.path(out_dir, "model", "deterministic"), showWarnings = F, recursive = T)
write_csv(monthlyFlows_agesex_domestic_hat_df, file.path(out_dir, "model", "deterministic", "mobilePhone_deterministic_agesex_domestic.csv"))
...
if(print_check){
  ...
  inflows = ifelse(is.na(inflows), 0, inflows),
      pop_minusOutflows_plusInflows = pop_updated + inflows,
      diff = monthlyFlow_hat_calibrated - pop_minusOutflows_plusInflows
    ) |> 
    View()
}
...
monthlyFlows_agesex_df |>
  filter(destination_hromada != "abroad" & des) |>
...
ggplot(borderCrossing_out_monthly, aes(x = t, y = outflows)) +
  geom_line() +
  theme_minimal() +
  labs(title = paste("Border crossings outflows (Border Services)"), x = "Time", y = "People")

ggplot(borderCrossing_out_monthly_agesex, aes(x = t, y = outflows, col = a_name, linetype = s_name)) +
  geom_line() +
  theme_minimal() +
  labs(title = paste("Border crossings outflows (Border Services + Eurostat)"), x = "Time", y = "People")
...
ggplot(borderCrossing_in_monthly_agesex, aes(x = t, y = inflows, col = a_name, linetype = s_name)) +
  geom_line() +
  theme_minimal() +
  labs(title = paste("Inflows"), x = "Time", y = "People")
...
p_abroad_mean <- monthlyFlows_agesex_domestic_hat_df |> 
  filter(origin_macroregion!='Abroad'&destination_macroregion!='Abroad')|> 
  left_join(
    monthlyFlows_agesex_df |>
      filter(origin_macroregion!='Abroad'&destination_macroregion!='Abroad')
  ...
  group_by(t, a_name, s_name) |>
  summarise(
    monthlyFlow_hat_calibrated = sum(monthlyFlow_hat_calibrated, na.rm = T),
  ...
  geom_hline(yintercept = 1, color = "grey20") +
  labs(
    title = paste("Penetration rate by age and sex of abroad as mean of domestic"),
    x = "Time", y = "mean(Subscribers/Pop)"
  )
...
penetration_rate_agesexMacroregion_df |>
  ...
  geom_hline(yintercept = 1, color = "grey20") +
  labs(
    title = paste("Penetration rate by age and sex of abroad as calibrated to border crossing"),
    x = "Time", y = "Subscribers/Border crossing"
  )

...
data {
  ...
  int<lower=0> K_pi; // number of covariates on proportional population change 
  ...
  array[T] matrix[I, K_pi] X_pi; // covariates on population proportions
...
parameters {
  ...
  real alpha_pi; // intercept for population proportions
  vector[K_pi] beta_pi; // covariate effects on population proportions
...
transformed parameters {
  ...
  array[T] vector[I] logit_pi;
  array[T] vector[I] mu_logit_pi;
  ...
  N[ti_N0] = N0;
  pi[1] = N0 / y_N_tot[1];
  logit_pi[1] = log(pi[1,1:I] / (1-pi[1,I]));
  mu_logit_pi[1] = alpha_pi + X_pi[1] * beta_pi;

  // t > 1
  for (t in 2 : T) {
    // free parameters of logit_pi vector for i=1:(I-1)
    logit_pi[t, 1:(I-1)] = logit_pi_free[t-1]; // t-1 because logit_pi_free does not include t=1
    
    // inverse-logit transform to derive pi
    pi[t] = softmax(append_row(logit_pi[t, 1:(I-1)], 1));
    
    // derive last element (I) of logit_pi vector from pi
    logit_pi[t, I] = log(pi[t,I] / (1-pi[t,I]));
    
    // covariate effects on proportions
    mu_logit_pi[t] = alpha_pi + X_pi[t] * beta_pi;
    
    // population proportional disaggregation
...
model {
  ...
  for (t in 1 : T) {
    logit_pi[t] ~ normal(mu_logit_pi[t], exp(log_sigma_pi));
  }
